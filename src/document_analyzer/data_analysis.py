...
class DocumentAnalyzer:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.parser = JsonOutputParser(pydantic_object=Metadate)
            self.fixing_parser = OutputFixingParser.from_llm(parser = self.parser,llm = self.llm)
            self.prompt = prompt
            self.log.info("DocumentAnalyzer initialized sucessfully")


        except Exception as e:
            self.log.error(f"Error Initialization DocumentAnalyzer:{e}")
